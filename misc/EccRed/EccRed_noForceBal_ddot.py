...
fs = 1 / (tdata[1] - tdata[0])  # Sampling frequency
cutoff = 0.05 #0.05  # Low cutoff frequency for smoothing
ddata_clean = butter_lowpass_filter(ddata, cutoff, fs)

# get time derivative
ddatadt = diff(tdata,ddata)

print("# t0 , d0 =", tdata[0], ",", ddata[0], ",", ddatadt[0])
print("# te , de =", tdata[-1], ",", ddata[-1]), ",", ddatadt[-1]
...
def dd_of_dt_model(t,A0,A1,B, omega,phi):
...
pinit = [d0, ddot,0, OmegaKepler, 0]

# use curve_fit from scipy
popt, pcov = optimize.curve_fit(dd_of_dt_model, tdata, ddatadt, p0=pinit)
#S0 = popt[0]
A0 = popt[0]
A1 = popt[1]
B  = popt[2]
...
for i in range(len(tdata)):
    print(tdata[i], ddatadt[i], dd_of_dt_model(tdata[i],  A0,A1,B, omega,phi))
...
def eval_pars(A0,A1,B, omega,phi,  r0, Omega):
    ecc = -B/(omega*r0)
    drdot = -B*np.sin(phi)
...
ecc, drdot, dOmega = eval_pars(A0,A1,B, omega,phi,  d0, Omega)
print("# ecc =", ecc)
print("# drdot  =", drdot)
print("# dOmega =", dOmega)
...
ecc, drdot, dOmega = eval_pars(A0,A1,B, omega,phi,  r0, Omega)
print("# ecc =", ecc)
print("# drdot  =", drdot)
print("# dOmega =", dOmega)
